...
- **`useChatId()`** from `@/providers/chat-id-provider` - Returns the current chat ID context (uses `parseChatIdFromPathname` internally)
- **`useIsSharedRoute()`** from `@/hooks/use-is-shared-route` - Returns `true` if on a `/share/:id` route
