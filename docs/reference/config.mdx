...
`lib/config.ts` is the central configuration for ChatJS. It's auto-generated from `lib/config.schema.ts` with sensible defaults and inline comments.

```bash
# Regenerate config.ts from schema (resets to defaults)
bun chatjs:init
```

<Note>
  In client components, use `useConfig()` from `@/components/config-provider`
  instead of importing `siteConfig` directly.
</Note>
