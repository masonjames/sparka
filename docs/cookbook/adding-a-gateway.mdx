...
description: How to add a new AI gateway provider to the multi-gateway system
---

Add support for any OpenAI-compatible API (or custom provider SDK) by implementing the `GatewayProvider` interface and registering it in the gateway system. This recipe walks through the full process.

## Problem

You want to route AI requests through a provider that is not built in (e.g., a local LLM server, a corporate proxy, or a niche cloud provider). The gateway abstraction lets you plug it in without touching application code.

## Architecture Overview

```mermaid
flowchart TB
    Config["chat.config.ts<br/>(gateway: 'my-gateway')"]
    Registry["gatewayRegistry<br/>lib/ai/gateways/registry.ts"]
    Schema["config-schema.ts<br/>(discriminated union)"]
    Impl["MyGateway class<br/>lib/ai/gateways/my-gateway.ts"]
    Env["lib/env.ts<br/>(env vars)"]

    Config -->|selects| Registry
    Registry -->|creates| Impl
    Schema -->|validates| Config
    Env -->|provides keys| Impl
```

Every gateway implements three things:

1. **`createLanguageModel(id)`** -- return a language model instance
2. **`createImageModel(id)`** -- return an image model (or `null` if unsupported)
3. **`fetchModels()`** -- list available models from the provider API
...
For providers with a dedicated SDK (like `@ai-sdk/openai` or `@openrouter/ai-sdk-provider`), install that instead. The `@ai-sdk/openai-compatible` package works with any provider that follows the OpenAI API format.
...
Create a new file at `lib/ai/gateways/my-gateway.ts`. Follow the pattern of the existing gateways:
...
Import your class and add it to the registry object:
...
The `satisfies` constraint ensures your class matches the `GatewayProvider` interface at compile time.

### 5. Add the schema variant

In `lib/config-schema.ts`, add your gateway to two places:

**The schema map** (compile-time enforced, so you get an error if you forget):
...
<Note>
The `gatewaySchemaMap` record is typed as `[G in GatewayType]`, so TypeScript will show an error if you add a gateway to the registry but forget to add it here.
</Note>
...
const config: ConfigInput = {
  ...
  models: {
    ...
    defaults: {
      ...
      pdf: "my-model-a",
      artifact: "my-model-a",
      artifactSuggestion: "my-model-a",
      followupSuggestions: "my-model-a",
      suggestions: "my-model-a",
      polishText: "my-model-a",
      formatSheet: "my-model-a",
      analyzeSheet: "my-model-a",
      codeEdits: "my-model-a",
      chatImageCompatible: "my-model-a",
      image: "my-image-model",
      deepResearch: "my-model-a",
      deepResearchFinalReport: "my-model-a",
    },
  },
};
```
...
## The GatewayProvider Interface

Every gateway must implement this interface:

```ts title="lib/ai/gateways/gateway-provider.ts"
...
The generic parameters give you type-safe model IDs. If your provider has a known set of model IDs, use the specific type (like the OpenAI gateway does with `OpenaiLanguageModelId`). Otherwise, use `string` (like the OpenRouter and OpenAI-compatible gateways).
...
sequenceDiagram
    ...
    App->>Registry: getGateway(config.gateway)
    Registry->>Gateway: new MyGateway()
    App->>Gateway: createLanguageModel("model-id")
    Gateway->>SDK: createOpenAICompatible(...)
    SDK-->>Gateway: provider instance
    Gateway->>SDK: provider("model-id")
    SDK-->>App: LanguageModel

    App->>Gateway: fetchModels()
    Gateway->>API: GET /models
    API-->>Gateway: model list
    Gateway-->>App: AiGatewayModel[]
```
...
| `lib/ai/gateways/*.ts` | Gateway implementations |
| `lib/config-schema.ts` | Zod schema with discriminated union |
| `lib/env.ts` | Environment variable validation |
| `chat.config.ts` | User-facing configuration |

## Checklist

When adding a new gateway, make sure you:
...
## Using the built-in OpenAI-compatible gateway

If your provider follows the OpenAI API format, you can skip creating a custom gateway and use the built-in `openai-compatible` gateway instead:
...
```bash title=".env.local"
OPENAI_COMPATIBLE_BASE_URL=https://api.my-provider.com/v1
OPENAI_COMPATIBLE_API_KEY=sk-...
```

This is the fastest way to connect to providers like [Ollama](https://ollama.com), [LM Studio](https://lmstudio.ai), [vLLM](https://docs.vllm.ai), [Azure OpenAI](https://azure.microsoft.com/products/ai-services/openai-service), or any other OpenAI-compatible endpoint.

## Related

- [Auto-Updating Models](/cookbook/auto-updating-models) for how `fetchModels()` feeds into the runtime model catalog
