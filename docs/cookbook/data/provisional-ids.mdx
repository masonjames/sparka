...
4. **URL updates** — Changes to `/chat/abc123` via History API (no reload)
5. **Components stay mounted** — Key unchanged, no remount
6. **User clicks "New Chat"** — `refreshChatID` generates new ID, components remount with fresh state
...
- [Resumable Streams](/cookbook/streaming/resumable-streams) for handling page refreshes during streaming
