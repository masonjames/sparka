...
<Note>
  For initial setup, see the [Quickstart](/quickstart) which covers the one-click deploy flow.
</Note>
...
By default, ChatJS uses [Vercel AI Gateway](/core/vercel-gateway) to access 120+ models from OpenAI, Anthropic, Google, and more.

1. Go to [Vercel AI Gateway](https://vercel.com/ai-gateway)
2. Create an API key
3. Add to environment: `AI_GATEWAY_API_KEY`

You can switch to [OpenRouter](/core/openrouter-gateway) by setting `gateway: "openrouter"` in `chat.config.ts`. See [Multi-Model Support](/core/multi-model) for configuring available models.
