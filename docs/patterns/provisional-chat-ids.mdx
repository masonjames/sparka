...
│                                                                 │
│  1. Visit Homepage (/)                                          │
│     └─> ChatIdProvider generates provisional ID: "abc123"       │
│     └─> ChatSystem mounts with key="abc123"                     │
│                                                                 │
│  2. User Types Message                                          │
│     └─> ChatInputProvider manages input state                   │
│     └─> ID is still "abc123" (provisional)                      │
│                                                                 │
│  3. User Submits Message                                        │
│     └─> Chat saved to database with ID "abc123"                 │
│     └─> confirmChatId("abc123") marks it as persisted           │
│     └─> URL updates to /chat/abc123 (no reload)                 │
│     └─> Components stay mounted (same key)                      │
│                                                                 │
│  4. User Clicks "New Chat"                                      │
│     └─> refreshChatID() generates new ID: "def456"              │
│     └─> URL updates to / (no reload)                            │
│     └─> ChatSystem remounts with key="def456" (fresh state)     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
- [Resumable Streams](/patterns/resumable-streams) for handling page refreshes during streaming
