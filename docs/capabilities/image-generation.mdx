...
Follows the [Tool Part](/cookbook/streaming/tool-part) pattern:
...
| ---- | ----------- | ------- |
| Image model | Standalone image generation models | `openai/gpt-image-1.5` |
| Multimodal | Language models with `image-generation` tag | `google/gemini-2.0-flash-exp` |
...
1. If the user's selected chat model can generate images (has `image-generation` tag), use it
2. Otherwise, fall back to `siteConfig.models.defaults.image`
