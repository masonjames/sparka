...
<Note>
  File attachments require `BLOB_READ_WRITE_TOKEN`. If you don't need attachments, set `integrations.attachments: false` in `lib/config.ts` to run ChatJS without this env var.
</Note>
...
Toggle attachments in `lib/config.ts`:
...
File attachment settings are configured in `lib/config.ts` under the `attachments` key:
