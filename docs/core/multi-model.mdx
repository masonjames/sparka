...
ChatJS provides access to hundreds of models from multiple providers through a pluggable gateway system. Set `gateway` in `chat.config.ts` to choose your backend:

| Gateway | Models | Setup |
|---------|--------|-------|
| [Vercel AI Gateway](/core/vercel-gateway) (default) | 120+ | `AI_GATEWAY_API_KEY` |
| [OpenRouter](/core/openrouter-gateway) | Hundreds | `OPENROUTER_API_KEY` |
...
The active gateway handles authentication with all providers. You do not need individual API keys for OpenAI, Anthropic, Google, etc. Instead, you authenticate with the gateway:
...
**OpenRouter**:
- `OPENROUTER_API_KEY` (obtain from [OpenRouter](https://openrouter.ai/keys))
...
See [Vercel Gateway](/core/vercel-gateway) and [OpenRouter Gateway](/core/openrouter-gateway) for detailed setup.
