...
All model settings are configured in `lib/config.ts` under the `models` key:
...
<Note>
Image generation uses a separate `defaults.image` setting. Language models with the `image-generation` tag can also generate images inline. See [Image Generation](/capabilities/image-generation) for details.
</Note>
