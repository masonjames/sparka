...
ChatJS provides access to hundreds of models from multiple providers through a pluggable [gateway system](/gateways/overview). Set `gateway` in `chat.config.ts` to choose your backend, then configure which models are available and how they behave.
