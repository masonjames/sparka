...
<Tabs>
  <Tab title="Vercel (Recommended)">
    ...
    cd YOUR_REPO
    bun install
    bun chatjs:init  # Generate lib/config.ts with defaults
    vercel link
  ...
  <Tab title="Manual Setup">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Blob Storage (Optional)">
        ...
        To disable, set `integrations.attachments: false` and `integrations.imageGeneration: false` in `lib/config.ts`.
      </Accordion>
...
<Accordion title="Optional Features">
  ...
  | `EXA_API_KEY`         | Web search           | [Exa](https://dashboard.exa.ai) |
  | `FIRECRAWL_API_KEY`   | Web scraping         | [Firecrawl](https://firecrawl.dev) |
...
<CardGroup cols={2}>
  <Card title="Configuration" icon="gear" href="/core/configuration">
    Customize features, branding, and integrations in lib/config.ts
  </Card>
