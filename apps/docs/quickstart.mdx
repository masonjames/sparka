...
The CLI walks you through selecting your AI gateway, features, and auth providers, then generates a configured project with `chat.config.ts` ready to go. It automatically detects your package manager and creates the appropriate lock file.
...
Expand the sections below to set up each service:
...
<AccordionGroup>
  ...
  <Accordion title="Authentication (at least one provider required)">
    **GitHub (Recommended)**
