...
The `chat.config.ts` file controls all feature toggles, branding, and app settings. It's generated with sensible defaults when you scaffold a project with `chatjs init` (or the `create-chat-app` alias).
  ...
  <Tab title="Vercel (Recommended)">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Google">
        7. Go to [Google Cloud Console](https://console.cloud.google.com/apis/credentials)
      ...
      <Accordion title="Vercel">
        6. Go to [Vercel Integration Console](https://vercel.com/docs/integrations/create-integration)
        7. Create a new integration with **Sign In with Vercel**
        8. Copy **Client ID** → `VERCEL_APP_CLIENT_ID`
        9. Copy **Client Secret** → `VERCEL_APP_CLIENT_SECRET`
      </Accordion>
    </AccordionGroup>
  ...
  <Tab title="Manual Setup">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Blob Storage (Optional)">
        ...
        To disable, set `features.attachments: false` and `features.imageGeneration: false` in `chat.config.ts`.
      </Accordion>
      <Accordion title="AI Gateway">
        ...
        **Option B: OpenRouter**
        ...
        4. Set `gateway: "openrouter"` in `chat.config.ts`

        See [Vercel Gateway](/gateways/vercel) and [OpenRouter Gateway](/gateways/openrouter) for details.
      </Accordion>
    </AccordionGroup>
    ...
    | `AUTH_SECRET` | [Generate one](https://generate-secret.vercel.app/32) |
    | `AI_GATEWAY_API_KEY` | From [Vercel AI Gateway](/gateways/vercel) (or `OPENROUTER_API_KEY` for [OpenRouter](/gateways/openrouter)) |
