...
The `chat.config.ts` file controls all feature toggles, branding, and app settings. It's generated with sensible defaults when you scaffold a project with `chatjs init` (or the legacy `create-chat-app` alias).
