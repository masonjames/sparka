...
<Frame>
  <img src="/images/ai-models.gif" alt="Selecting AI models in the chat interface" />
</Frame>
...
| `deepResearch`              | Research and supervision agent (see [Deep Research](/features/deep-research)) |
| `deepResearchFinalReport`   | Final report synthesis for deep research                              |
...
Set the default image model in `defaults.image` (gateway-specific model ID). The `generateImage` tool resolves the model at runtime: if the user's selected chat model supports image output (per the app model registry), it uses that; otherwise it uses `config.models.defaults.image`. Resolution uses the dynamic gateway-based registry, not a static snapshot. See [Image Generation](/features/image-generation) for details.
